...
static void _MIDI_ServiceRoutine
...
#ifdef HAVE_CAMD
   // Find the smallest delay among the active tracks
   int minDelay = 0x7FFFFFFF;
   Track = _MIDI_TrackPtr;
   for (tracknum = 0; tracknum < _MIDI_NumTracks; tracknum++)
   {
      if (Track->active)
      {
         if (Track->delay < minDelay)
         {
   ...
   // Advance the ticks if needed and allow the service thread to sleep longer
   int numIdleTics = 1;
   if (/*_MIDI_ActiveTracks > 0 &&*/ minDelay > 0)
   {
      Track = _MIDI_TrackPtr;
      for (tracknum = 0; tracknum < _MIDI_NumTracks; tracknum++)
      {
      ...
      numIdleTics += minDelay;
   }
   _MIDI_PositionInTicks += numIdleTics;
   _MIDI_Time += _MIDI_FPSecondsPerTick*numIdleTics;
   _MIDI_Tick += numIdleTics;
   while( _MIDI_Tick > _MIDI_TicksPerBeat )
      {
      _MIDI_Tick -= _MIDI_TicksPerBeat;
      _MIDI_Beat++;
   ...
   // TODO pass this via a driver function
   extern int g_numIdleTics;
   g_numIdleTics = numIdleTics;
#else
   _MIDI_AdvanceTick();
#endif
   }
