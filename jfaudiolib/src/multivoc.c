...
#ifdef HAVE_MPEG
      case MV_InvalidMPEGFile :
         ErrorString = "Invalid MPEG file passed in to Multivoc.";
         break;
#endif
...

{
   ...
   if (voice->wavetype == MPEG)
      {
      MV_PauseMPEGVoice(voice, pauseon);
      }
   #endif
...
void MV_SetVoiceVolume
   ...
   if (voice->wavetype == MPEG)
      {
      MV_SetMPEGVolume(voice, vol, left, right);
      return;
      }
   #endif
...
#ifdef __AMIGA__
   if ( ASS_PCMSoundDriver == ASS_Paula && (MV_MixMode & STEREO) != 0 )
      {
      // Mix the buffers in a de-interleaved format
      MV_RightChannelOffset = MV_BufferSize / 2;
