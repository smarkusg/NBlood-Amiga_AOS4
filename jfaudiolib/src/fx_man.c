                 ...
                 int left, int right, int priority, unsigned int callbackval )
{
   ...
   #ifdef HAVE_MPEG
   } else if (!memcmp("ID3", ptr, 3) ||
      (((unsigned char *)ptr)[0] == 0xFF && (((unsigned char *)ptr)[1] & 0xF0) == 0xF0) ||
      (!memcmp("RIFF", ptr, 4) && !memcmp("WAVE", ptr + 8, 4) && ptr[19] == 0 && ptr[20] == 0x55)) {
      handle = MV_PlayMPEG(ptr, length, pitchoffset, vol, left, right, priority, callbackval);
   #endif
                       ...
                       unsigned int callbackval )
{
   ...
   if (!memcmp("Creative Voice File\x1a", ptr, 20)) {
      ...
      (((unsigned char *)ptr)[0] == 0xFF && (((unsigned char *)ptr)[1] & 0xF0) == 0xF0) ||
      (!memcmp("RIFF", ptr, 4) && !memcmp("WAVE", ptr + 8, 4) && ptr[19] == 0 && ptr[20] == 0x55)) {
      handle = MV_PlayLoopedMPEG(ptr, length, loopstart, loopend, pitchoffset,
                              vol, left, right, priority, callbackval);
   #endif
   } else if (!memcmp("RIFF", ptr, 4) && !memcmp("WAVE", ptr + 8, 4)) {
