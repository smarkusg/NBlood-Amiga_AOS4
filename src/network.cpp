...
#ifdef __AMIGA__
#define CHECKSUM_NO_COPY
#define GETPACKETS_NO_COPY
#endif
void netGetPackets(void)
{
    ...
    short nSize;
    char buffer[128];
#if defined(GETPACKETS_NO_COPY) && !defined(NETCODE_DISABLE)
    if (gNetMode == NETWORK_NONE)
        return;
    netUpdate();
    while (gNetPacketTail != gNetPacketHead)
#else
...
#endif
    {
        ...
        ENetPacket *pEPacket = gNetPacketFifo[gNetPacketTail];
        gNetPacketTail = (gNetPacketTail+1)%kENetFifoSize;
        char *pPBuffer = (char*)pEPacket->data;
        nPlayer = GetPacketByte(pPBuffer);
        nSize = pEPacket->dataLength-1;
        char *pPacket = pPBuffer;
#else
        char *pPacket = packet;
#endif
...
#if defined(GETPACKETS_NO_COPY) && !defined(NETCODE_DISABLE)
            while (pPacket < pPBuffer+nSize)
#else
            while (pPacket < packet+nSize)
#endif
            {
#ifdef CHECKSUM_NO_COPY
                int *checkSum = (int *)pPacket;
                pPacket += 16;
#else
                int checkSum[4];
                ...
                for (int p = connecthead; p >= 0; p = connectpoint2[p])
                {
                    ...
                    {
#ifdef CHECKSUM_NO_COPY
                        memcpy(gCheckFifo[gCheckHead[p]&255][p], checkSum, 16);
#else
        ...
        case 1: // From slave
        {
...
#endif
            {
                ...
                int *checkSum = (int *)pPacket;
                pPacket += 16;
                memcpy(gCheckFifo[gCheckHead[nPlayer]&255][nPlayer], checkSum, 16);
#else
                int checkSum[4];
                GetPacketBuffer(pPacket, checkSum, sizeof(checkSum));
...
void netGetInput(void)
{
    ...
    input = gNetInput;
#ifdef __AMIGA__
    gNetInput.keyFlags.word = 0;
    gNetInput.newWeapon = 0;
    /*
    USEFLAGS useFlags;
    */
#endif
...
//#define USE_MEM_STAT
#ifdef USE_MEM_STAT
static unsigned int maxAllocSize = 0;
static unsigned int highestAllocSize = 0;
static unsigned int maxAllocCount = 0;
static unsigned int allocCount = 0;
static unsigned int allocSize = 0;
#define STAT_ALLOC(size) do { if ((size) > maxAllocSize) maxAllocSize = (size); allocSize += (size); if (allocSize > highestAllocSize) highestAllocSize = allocSize; allocCount++; } while(0)
#define STAT_FREE(size) do { allocCount--; allocSize -= (size); } while(0)
#define STAT_PRINT() buildprintf("%s:%d maxAllocSize %d highestAllocSize %d allocCount %d allocSize %d\n", __FUNCTION__, __LINE__, maxAllocSize, highestAllocSize, allocCount, allocSize)
#define STAT_RESET() do { maxAllocSize = highestAllocSize = maxAllocCount = allocCount = allocSize = 0; } while(0);
#else
#define STAT_ALLOC(size)
#define STAT_FREE(size)
#define STAT_PRINT()
#define STAT_RESET() 
#endif
...
#define PUDDLESIZE (32*1024)
#define THRESHSIZE (10*1024)
static void pool_init()
{
    memPool = CreatePool(MEMF_FAST, PUDDLESIZE, THRESHSIZE);
    STAT_RESET();
}
...
{
    return AllocVecPooled(memPool, size);
}

static void pool_free(void *memory)
{
    FreeVecPooled(memPool, memory);
}
#endif

//static BYTE oldPri = 0;
#endif
...
#ifdef __AMIGA__
    // give TCP/IP stack some room to breathe
    oldPri = SetTaskPri(FindTask(NULL), -1);
    ENetCallbacks inits;
    memset(&inits, 0, sizeof(inits));
    pool_init();
    inits.malloc = pool_malloc;
    inits.free = pool_free;
    inits.no_memory = QuitGame;
    if (enet_initialize_with_callbacks(enet_linked_version(), &inits) != 0)
#else
...
#ifdef __AMIGA__
static ClockTicks lastNetUpdateClock = 0;
#endif
