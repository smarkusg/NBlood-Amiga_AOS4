	...
	for(v=npoints-1;v>=0;v--)
	{
			...
			y1 = nry1[v]; y2 = nry1[nextv];
		{
			...
			if (dax2 > dax1)
				...
				{
					yplc = y1;
					for(i=dax1;i<=dax2;i++) uplc[i] = yplc>>16;
				}
				else
				{
#endif
			...
			else
			{
				...
				{
					yplc = y2;
					for(i=dax2;i<=dax1;i++) dplc[i] = yplc>>16;
				}
				else
				{
#endif
...
void buildprintf(const char *fmt, ...)
{
	...
	va_copy(vac, va);
#ifndef __AMIGAOS4__ //markus disable condole log fixme !!!!
...
{
#ifndef __AMIGAOS4__
    fputs(str, stdout); //markus disable condole log fixme !!!!
#endif
